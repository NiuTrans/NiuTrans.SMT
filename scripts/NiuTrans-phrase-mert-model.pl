...
my $logo =   "########### SCRIPT ########### SCRIPT ############ SCRIPT ##########\n".
             "#                                                                  #\n".
             "#   NiuTrans Phrase MERT (version 0.3.0 Beta)   --www.nlplab.com   #\n".
             "#                                                                  #\n".
...
my $log;
unlink $log;
open( $log, ">".$param{ "-l" } ) or die "Error: can not open file $log!\n";
print $log $logo;
...
#                    my $exec = "../bin/NiuTrans.Decoder -mert $param{ \"-dev\" } -config $param{ \"-c\" } 1 $param{ \"-nref\" } $param{ \"-ngram\" } $param{ \"-nthread\" }";
                    my $exec = "../bin/NiuTrans.Decoder -mert $param{ \"-dev\" } 1 -config $param{ \"-c\" }";
...
sub getParameter
{
          ...
          {
                    print STDERR "[USAGE]     :\n".
                                 "        NiuTrans-phrase-mert-model.pl    [OPTIONS]\n".
                                 "[OPTION]    :\n".
                                 "    -c      :  Mert Config File.\n".
                                 ...
                                 "[EXAMPLE]   :\n".
                                 "    perl NiuTrans-phrase-mert-model.pl -c config-file -dev dev-set\n";
          ...
          if( !exists $param{ "-c" } )
          {
                    print STDERR "Error: please assign -c!\n";
                    exit( 1 );
          }

          if( !exists $param{ "-l" } )
          {
                    $param{ "-l" } = "mert-model.log";
          }

          $param{ "-r" } = 2 if( !exists $param{ "-r" } );
